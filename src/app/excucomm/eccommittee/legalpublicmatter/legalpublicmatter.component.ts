...
@Component({
  selector: 'app-legalpublicmatter',
  templateUrl: './legalpublicmatter.component.html',
  styleUrls: ['./legalpublicmatter.component.css']
})
export class LegalpublicmatterComponent implements OnInit,OnDestroy {
  rowId = 0;
  pageId = 10;
  pageCount: any;
  legalMatterList: any = [];
  ...
  legalMatter() {
    ...
    this.ecservice.basedAgendaList(meetingId, 9, this.rowId, this.pageId)
    .pipe(takeUntil(this.subscribe)).subscribe((data: any) => {
      console.log(data);
      if (data.length !== 0) {
        this.legalMatterList = data;
        this.pageCount = Math.ceil(this.legalMatterList[0].totalCount / this.pageId);
      } else {
        this.legalMatterList = [];
        this.pageCount = 0;
      }
    }, error => {
      console.log(error);
